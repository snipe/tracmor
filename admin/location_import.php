 ...
 */
    ...
    protected $intLocationDescriptionKey;
    ...
    protected $lblImportLocations;
        ...
        $file = fopen(__TRACMOR_TMP__."/".$_SESSION['intUserAccountId']."_location_skipped.csv", "r");
        ob_end_clean();
      ...
      $this->Labels_Create();
			...
			// Load Custom Field (Locations have no custom fields now)
    ...
    protected function Labels_Create() {
      ...
      $this->lblImportLocations->HtmlEntities = false;
      $this->lblImportLocations->Display = false;
      $this->lblImportLocations->CssClass = "title";
      $this->lblImportLocations->Text = "<br/><br/>Last Imported Locations";
    }
      ...
      $this->pnlStepOne->Template = "location_import_pnl_step1.tpl.php";
    ...
    protected function pnlStepTwo_Create() {
      ...
      $this->pnlStepTwo->Template = "location_import_pnl_step2.tpl.php";
    }

    // Step 3 Panel
    protected function pnlStepThree_Create() {
			$this->pnlStepThree = new QPanel($this->pnlMain);
      $this->pnlStepThree->Template = "location_import_pnl_step3.tpl.php";
    }
      ...
      switch ($this->lstTextDelimiter->SelectedValue) {
		...
		protected function btnNext_Click() {
          ...
          // File Not Uploaded
            ...
            while ($row = fgets($file, 1000)) {
              if ($j == 1) {
                $strFilePath = __TRACMOR_TMP__.'/'.$_SESSION['intUserAccountId'].'_loc_'.$i.'.csv';
            ...
            if (true && QApplication::$TracmorSettings->AssetLimit != null && QApplication::$TracmorSettings->AssetLimit < ($this->intTotalCount + Asset::CountAll())) {
              ...
              $file_skipped = fopen($this->strFilePath = __TRACMOR_TMP__.'/'.$_SESSION['intUserAccountId'].'_location_skipped.csv', "w+");
              // Get Headers
              if ($this->blnHeaderRow) {
        ...
        */
        ...
        for ($i=0; $i < count($this->lstMapHeaderArray)-1; $i++) {
          ...
          if ($strSelectedValue == "location name") {
            $blnLocation = true;
          }
        ...
        if (!$blnError && $blnLocation && ($this->lstImportAction->SelectedValue != 2 || $blnLocationId)) {
          ...
          foreach ($this->arrTracmorField as $key => $value) {
            ...
            elseif ($value == 'location description') {
              $this->intLocationDescriptionKey = $key;
            }
          ...
          $this->dtgUpdatedItems->Name = 'updated_location_list';
          ...
          $this->dtgUpdatedItems->AddColumn(new QDataGridColumnExt('location name', '<?= $_ITEM ?>', 'CssClass="dtg_column"', 'HtmlEntities="false"'));
          ...
          $blnError = true;
		    ...
		    // Step 3 complete
		    set_time_limit(0);
		    $file_skipped = fopen($strFilePath = __TRACMOR_TMP__.'/'.$_SESSION['intUserAccountId'].'_location_skipped.csv', "a");
          ...
          for ($j=$this->intCurrentFile; $j<count($this->strFilePathArray); $j++) {
            ...
            if ($this->intImportStep == 2) {
              ...
              */
              ...
              $strUpdatedLocationValuesArray = array();
              ...
              $objLocationArray = array();
              foreach (Location::LoadAll() as $objLocation) {
                $objLocationArray[strtolower($objLocation->ShortDescription)] = $objLocation;
              }
                  ...
                  if (!$blnError) {
                    ...
                    if (isset($this->intLocationDescriptionKey))
                      if (trim($strRowArray[$this->intLocationDescriptionKey]))
                        ...
                        $strLocationDescription = (isset($this->txtMapDefaultValueArray[$this->intLocationDescriptionKey])) ? trim($this->txtMapDefaultValueArray[$this->intLocationDescriptionKey]->Text) : '';
                    $this->arrOldItemArray[$objLocation->LocationId] = $objLocation;
                    $strUpdatedLocationValuesArray[] = sprintf("UPDATE `location` SET `short_description`='%s', `long_description`='%s' WHERE `location_id`='%s'", addslashes(trim($strRowArray[$this->intLocationKey])), addslashes($strLocationDescription), $objLocation->LocationId);
                    $this->objUpdatedItemArray[$objLocation->LocationId] = $objLocation->ShortDescription;
              ...
              if (count($strLocationValuesArray)) {
                ...
                $objDatabase->NonQuery(sprintf("INSERT INTO `location` (`short_description`, `long_description`, `created_by`, `creation_date`) VALUES %s;", implode(", ", $strLocationValuesArray)));
                $intStartId = $objDatabase->InsertId();
                $strItemIdArray = array();
                for ($i=0; $i<count($strNewLocationArray); $i++) {
                  $this->objNewLocationArray[$intStartId+$i] = $strNewLocationArray[$i];
                  $objDatabase = CustomField::GetDatabase();
                ...
                if (count($strItemCFVArray) > 0 && count($strCFVNameArray) > 0)  {
                	$strQuery = sprintf("INSERT INTO `location_custom_field_helper` (`location_id`, %s) VALUES %s", implode(", ", $strCFVNameArray), implode(", ", $strItemCFVArray));
                } else {
                	$strQuery = sprintf("INSERT INTO `location_custom_field_helper` (`location_id`) VALUES %s", implode(", ", $strItemIdArray));
                }
                $objDatabase->NonQuery($strQuery);*/
              }
              if (count($strUpdatedLocationValuesArray)) {
                $objDatabase = Location::GetDatabase();
                foreach ($strUpdatedLocationValuesArray as $query) {
          ...
          if ($this->intImportStep == 3) {
            ...
            $this->lblImportSuccess->Text = sprintf("Success:<br/>" .
                                             ...
                                             "<b>%s</b> Records skipped due to error<br/>", count($this->objNewLocationArray) + count($this->objUpdatedItemArray), $this->intSkippedRecordCount);
            if ($this->intSkippedRecordCount) {
               $this->lblImportSuccess->Text .= sprintf("<a href='./location_import.php?intDownloadCsv=1'>Click here to download records that could not be imported</a>");
            }
    ...
    protected function in_array_nocase($search, &$array) {
	  ...
	  protected function lstMapHeader_Create($objParentObject, $intId, $strName = null) {
	    ...
	    $strLocationGroup = "Location";
	    $lstMapHeader->AddItem("- Not Mapped -", null);
	    //$lstMapHeader->AddItem("Location", "Location", ($strName == 'location') ? true : false, $strLocationGroup, 'CssClass="redtext"');
	    ...
	    /*foreach ($this->arrItemCustomField as $objCustomField) {
	      $lstMapHeader->AddItem($objCustomField->ShortDescription, "location_".$objCustomField->CustomFieldId,  ($strName == strtolower($objCustomField->ShortDescription)) ? true : false, $strLocationGroup);
	    }*/
      ...
      if ($objControl->SelectedValue != null) {
        ...
        if ($this->in_array_nocase($search, $this->arrTracmorField)) {
          ...
          /*if (substr($objControl->SelectedValue, 0, 9) == "location_") {
    ...
    protected function UndoImport() {
      ...
      if (isset($this->arrOldItemArray))
        foreach ($this->arrOldItemArray as $intItemId => $objOldItem) {
          $strQuery = sprintf("UPDATE `location` SET `short_description`='%s', `long_description`='%s' WHERE `location_id`='%s'", $objOldItem->ShortDescription, $objOldItem->LongDescription, $objOldItem->LocationId);
          $objDatabase->NonQuery($strQuery);
          /*$strCFVArray = array();
          foreach ($this->arrItemCustomField as $objCustomField) {
          ...
          $strQuery = sprintf("UPDATE `location_custom_field_helper` SET %s WHERE `location_id`='%s'", implode(", ", $strCFVArray), $intItemId);
          $objDatabase->NonQuery($strQuery);*/
      }
		  if (count($this->objNewLocationArray)) {
