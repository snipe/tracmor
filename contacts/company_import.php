 ...
 */
  ...
  require_once('../includes/prepend.inc.php');
    ...
    protected $blnError;
        ...
        $file = fopen(__TRACMOR_TMP__."/".$_SESSION['intUserAccountId']."_company_skipped.csv", "r");
        ob_end_clean();
      ...
      $this->Labels_Create();
			$this->objDatabase = Company::GetDatabase();
			// Load Custom Field
      ...
      switch ($this->lstTextDelimiter->SelectedValue) {
		...
		protected function btnNext_Click() {
          ...
          // File Not Uploaded
            ...
            while ($row = fgets($file, 1000)) {
              if ($j == 1) {
                $strFilePath = __TRACMOR_TMP__.'/'.$_SESSION['intUserAccountId'].'_com_'.$i.'.csv';
            ...
            if (true && QApplication::$TracmorSettings->AssetLimit != null && QApplication::$TracmorSettings->AssetLimit < ($this->intTotalCount + Asset::CountAll())) {
              ...
              $file_skipped = fopen($this->strFilePath = __TRACMOR_TMP__.'/'.$_SESSION['intUserAccountId'].'_company_skipped.csv', "w+");
              // Get Headers
              if ($this->blnHeaderRow) {
        ...
        if (!$blnError && $blnCompany && ($this->lstImportAction->SelectedValue != 2 || $blnCompanyId)) {
          ...
          $blnError = true;
		    ...
		    // Step 3 complete
		    set_time_limit(0);
		    $file_skipped = fopen($strFilePath = __TRACMOR_TMP__.'/'.$_SESSION['intUserAccountId'].'_company_skipped.csv', "a");
