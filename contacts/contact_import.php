 ...
 */
  ...
  require_once('../includes/prepend.inc.php');
    ...
    protected $blnError;
        ...
        $file = fopen(__TRACMOR_TMP__."/".$_SESSION['intUserAccountId']."_contact_skipped.csv", "r");
        ob_end_clean();
      ...
      switch ($this->lstTextDelimiter->SelectedValue) {
		...
		protected function btnNext_Click() {
          ...
          // File Not Uploaded
            ...
            while ($row = fgets($file, 1000)) {
              if ($j == 1) {
                $strFilePath = __TRACMOR_TMP__.'/'.$_SESSION['intUserAccountId'].'_con_'.$i.'.csv';
            ...
            if (true && QApplication::$TracmorSettings->AssetLimit != null && QApplication::$TracmorSettings->AssetLimit < ($this->intTotalCount + Asset::CountAll())) {
              ...
              $file_skipped = fopen($this->strFilePath = __TRACMOR_TMP__.'/'.$_SESSION['intUserAccountId'].'_contact_skipped.csv', "w+");
              // Get Headers
              if ($this->blnHeaderRow) {
        ...
        if (!$blnError && $blnCompany && $blnLastName && ($this->lstImportAction->SelectedValue != 2 || $blnContactId)) {
          ...
          $blnError = true;
		    ...
		    // Step 3 complete
		    set_time_limit(0);
		    $file_skipped = fopen($strFilePath =  __TRACMOR_TMP__.'/'.$_SESSION['intUserAccountId'].'_contact_skipped.csv', "a");
